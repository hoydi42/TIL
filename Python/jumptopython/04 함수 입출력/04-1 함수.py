...
-----------------------------
# vartest.py
a = 1       #  a라는 변수를 생성하고 1을 대입
def vartest(a): #입력으로 들어온 값에 1을 더해 주고 
    a = a +1    #결괏값은 돌려주지 않는 vartest 함수를 선언

vartest(a)  #vartest 함수에 입력값으로 a를 줌
print(a)    #a의 값을 출력하는 print(a)를 입력
------------------------------
vartest 함수에서 매개변수 a의 값에 1을 더했으니까 2가 출력될 것 같지만 
프로그램 소스를 작성해서 실행해 보면 결괏값은 1이 나옴.
함수 안에서 새로 만든 매개변수는 함수 안에서만 사용하는 "함수만의 변수"이기 때문
def vartest(a)에서 입력값을 전달받는 매개변수 a는 
함수 안에서만 사용하는 변수이지 함수 밖의 변수 a가 아니라는 뜻
함수 안에서 사용하는 매개변수는 함수 밖의 변수 이름과는 전혀 상관이 없다
----------------------
def vartest(hello):
    hello = hello + 1
hello 를 넣으나 a를 넣으나 똑같은 함수
-----------------------
...
물론 vartest 함수 안의 a 매개변수는 함수 밖의 a와는 다른 것



2. global 명령어 사용하기 (bad)
...
lambda 는 함수를 생성할 때 사용하는 예약어로 def 와 동일한 역할
